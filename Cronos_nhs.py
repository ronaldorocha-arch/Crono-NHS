...
CORES_POSTOS = ["#00bcd4", "#4caf50", "#e040fb", "#ff9800", "#9c27b0",
                "#f44336", "#2196f3", "#8bc34a", "#ff5722", "#607d8b"]
...
st.markdown("""
    ...
    @media print {
        ...
        header, footer, .stApp > header, .stTabs [data-baseweb="tab-list"],
        #MainMenu, [data-testid="stSidebar"], h1, .no-print,
        [data-testid="stMultiSelect"], [data-testid="stSelectbox"],
        [data-testid="stRadio"], .stExpander {
        ...
        * {
        ...
        /* Garante que tabela HTML não seja cortada */
        .tabela-cap {
            page-break-inside: avoid !important;
        }
    }
    ...
    .b-6 { background-color: #f44336; } .b-7 { background-color: #2196f3; } .b-8 { background-color: #8bc34a; } .b-9 { background-color: #ff5722; } .b-10 { background-color: #607d8b; }
...
with tab_dash:
    ...
    if not df_tp.empty:
        st.markdown("<div class='no-print' style='background:#eef7ff; padding:15px; border-radius:5px; border:1px solid #b3d4fc; margin-bottom:15px;'><b style='color:#0056b3; font-size: 15px;'>🖨️ Tamanho da Impressão (Ctrl+P)</b><br><span style='font-size: 13px; color: #555;'>Selecione a folha abaixo antes de imprimir. O sistema ajustará o zoom automaticamente.</span></div>", unsafe_allow_html=True)
        tam_folha = st.radio("Selecione o tamanho:", ["A3", "A4"], horizontal=True, label_visibility="collapsed")

        # FIX 1: Zoom reduzido + overflow visible para evitar cortes na impressão
        if tam_folha == "A4":
            st.markdown("""
                <style>
                @media print {
                    ...
                    .stApp, .block-container, [data-testid="stHorizontalBlock"], [data-testid="column"] {
                        overflow: visible !important;
                    }
        ...
        if not df_f.empty:
            ...
            # FIX 2: Mapa de cores por posto — igual à Carta de Trabalho
            postos_ordenados = sorted(df_f['Posto'].unique(), key=extrair_numero_posto)
            color_map_postos = {
                p: CORES_POSTOS[(extrair_numero_posto(p) - 1) % len(CORES_POSTOS)]
        ...
        with col_sup_esq:
            ...
            with c_layout_desenho:
                ...
                for i, p_nome in enumerate(postos_display):
                    ...
                    idx_cor = ((extrair_numero_posto(p_nome) - 1) % len(CORES_POSTOS)) + 1
                    ...
                    if tem_andon:
                        html_posto += "<div class='andon'></div>"
                    if tem_flow:
                        html_posto += "<div class='ponto-uso'>PONTO DE USO</div>"
                    if wip > 0:
                        html_posto += f"<div class='wip-badge'>{wip}</div>"

                    if pos_op == 'Trás': html_posto += "<div class='operador-icon op-tras'>👤</div>"
                    elif pos_op == 'Esquerda': html_posto += "<div class='operador-icon op-esq'>👤</div>"
                    elif pos_op == 'Direita': html_posto += "<div class='operador-icon op-dir'>👤</div>"
                    else: html_posto += "<div class='operador-icon op-frente'>👤</div>"
        ...
        with col_sup_dir:
            ...
            if not df_f.empty:
                ...
                for idx_p, p_nome in enumerate(postos_gbo):
                    with cols_pizza[idx_p]:
        ...
        with col_inf_esq:
            ...
            if not df_f.empty:
                # FIX 2: color_discrete_map explícito — cores idênticas à Carta de Trabalho
                fig_gantt = px.bar(
                    df_f, x="Tempo (s)", y="Passo_Unico", base="Início (s)",
                    color="Posto", orientation='h', text="Tempo (s)",
                    color_discrete_map=color_map_postos
                )
